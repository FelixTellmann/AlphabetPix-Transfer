...
theme.Create = (function () {
    ...
    function Create(container) {
        ...
        const twitter_link = $('.social-sharing__link--twitter');
        const pinterest_link = $('.social-sharing__link--pinterest');
        ...
        const frame = {
            ...
            image_array_4: $('.create__step-4 .frame__letter'),
            label: $('.frame__label'),
            background: {
        ...
        let create_data = {
            ...
            artwork_image_src: '',
            facebook_sharing_url: '',
            twitter_sharing_url: '',
            pinterest_sharing_url: '',
            modal: {
        ...
        let change_letter_image = function (position, letter = false, theme_data, variant_id = false) {
            ...
            if (variant_id) {
                ...
                image_4 = $('#create__modal .preload-image[data-variant-id="' + variant_id + '"]').clone(false).attr('data-selection-type', 'selected');
            } else if (letter) {
                ...
                image_4 = $('#create__modal .preload-image[data-letter-id="' + letter + '"][data-theme-id="' + theme_data + '"]').clone(false).attr('data-selection-type', 'auto');
            }
            ...
            frame.image_array_4.eq(position).empty().append(image_4[0]);
        };
        ...
        let change_steps = function (target_step) {
            ...
            if (target_step > 1 && create_data.input_array.length >= 3) {
                $('.shopify-section:not(.create-section):not(.header-section)').attr('style', 'display:none');
                $('.announcement-bar').attr('style', 'height: 0');
                $('.shopify-section.header-section').attr('style', 'opacity: 0.4; pointer-events: none;');
                $('body').attr('style', 'overflow-y: scroll');
            } else {
                $('.shopify-section:not(.create-section)').attr('style', '');
                $('.announcement-bar').attr('style', '');
                $('.shopify-section.header-section').attr('style', '');
                $('body').attr('style','');
            }
        };

        /*================ update_tabindex(2) ================*/
        let update_tabindex = function (target_step) {
            $('.create__step-' + target_step).focus();
            $('.create__slider [tabindex]').attr('tabindex', '-1');
            $('.create__step-' + target_step + ' [tabindex]').attr('tabindex', '0');
            $.each($('button.frame__letter[tabindex]'), function (i, letter_container) {
                if (letter_container.innerHTML.length === 0) {
                    $(letter_container).attr('tabindex', '-1');
                }
            });
        };
        ...
        let show_reset_button = function (input_length) {
            if (input_length > 0) {
                create_navigation.reset_button_1.removeClass('button--hidden').attr('tabindex', '0');
            } else {
                create_navigation.reset_button_1.addClass('button--hidden').attr('tabindex', '-1');
            }
        };
        ...
        let collect_input_cart_data = function (input_array = create_data.input_array, variant_id = false, theme_data = create_data.theme, frame_data = create_data.frame, artwork_image_src = create_data.artwork_image_src) {
            ...
            create_data.input_word = input_array.join('');
            ...
            create_data.sharing_url = 'https://alphabetpix.myshopify.com/?frame=' + create_data.frame + '&word=' + create_data.input_array.join('');
            $.each(create_data.input_array, function (i) {
                if (create_data.input[i].custom_image !== '') {
                    create_data.sharing_url += '&pos-' + i + '=' + create_data.input[i].variant_id + '||||' + encodeURIComponent(create_data.input[i].custom_image);
                } else {
                    create_data.sharing_url += '&pos-' + i + '=' + create_data.input[i].variant_id;
                }
            });

            update_sharing_links();
        ...
        /*================Update_sharing_links(create_data) ================*/
        let update_sharing_links = function () {
            create_data.facebook_sharing_url = '//www.facebook.com/sharer.php?u=' + create_data.sharing_url;
            create_data.twitter_sharing_url = '//twitter.com/share?text=' + window.page_description + '&amp;url=' + create_data.sharing_url;
            create_data.pinterest_sharing_url = '//pinterest.com/pin/create/button/?url=' + create_data.sharing_url + '&amp;media=' + create_data.artwork_image_url + '&amp;description=' + window.page_description;

            final_artwork.sharing_link.attr('href', create_data.sharing_url).html(create_data.sharing_url);
            facebook_link.attr('href', create_data.facebook_sharing_url);
            twitter_link.attr('href', create_data.twitter_sharing_url);
            pinterest_link.attr('href', create_data.pinterest_sharing_url);
        };
        ...
        let create_final_artwork = function () {
            if (create_data.input_array.length >= 3) {
                collect_input_cart_data(create_data.input_array, false, create_data.theme, create_data.frame);

                try {
                    domtoimage.toJpeg(fixed_size_frame[0], {quality: 0.4}).then(function (dataUrl) {
                        final_artwork.container.html('<img src="' + dataUrl + '" alt="Your Unique Artwork - ' + create_data.input_word + '">');
                        collect_input_cart_data(create_data.input_array, false, create_data.theme, create_data.frame, dataUrl);
                        postImageToFacebook("EAAFEyUKxbkMBADE2HTXZAZALfOpHnjM5ZAF08AUFPM34jF7wNedf2vKBzvHY8jFPyNZA7GmCZABBtTQ3M2KGBVrix0wP2FX2ulUCge6dGbjodMhGTZA7Xe7v40ABLxZC3pZAVmvikOFHDV6gCwMtRAASWVUvEc2bJxrwzvQPNHWnkEJS6SgKItB8", "Canvas to Facebook/Twitter", "image/png", dataURItoBlob(dataUrl), window.location.href);
                        download_link.attr('download', 'My personal frame - ' + create_data.input_array.join('')).attr('href', dataUrl).removeClass('visually-hidden');
                    });
                }
                catch (err) {
                    ...
                    FB.api(
                      ...
                      function (response) {
                          if (response && !response.error) {
                              ...
                              create_data.sharing_url = create_data.sharing_url + '&image_url=' + encodeURIComponent(create_data.artwork_image_url);
                              update_sharing_links();
                              window.sessionStorage.create_data = JSON.stringify(create_data);
        ...
        let preload_artwork = function (url_values) {
            if (Object.keys(url_values).length > 1 && 'frame' in url_values && 'word' in url_values) {
                ...
                show_reset_button(url_values.word.length);
